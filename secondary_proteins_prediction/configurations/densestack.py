...
def build_model():
    ...
    l_dim_b = lasagne.layers.DimshuffleLayer(l_conv_a_b, (0, 2, 1))
    l_reshape_b = lasagne.layers.ReshapeLayer(l_dim_b, (batch_size * seq_len, N_CONV_A))

    # 2.5. Dense layer
    l_2 = lasagne.layers.DenseLayer(l_reshape_b, num_units=N_L1, nonlinearity=lasagne.nonlinearities.rectify)
    ...
    # 4. Second Conv layer
    l_dim_a = lasagne.layers.DimshuffleLayer(l_out, (0, 2, 1))
    l_conv_a = lasagne.layers.Conv1DLayer(
    ...
    l_dim_b = lasagne.layers.DimshuffleLayer(l_conv_a_b, (0, 2, 1))
    l_reshape_b = lasagne.layers.ReshapeLayer(l_dim_b, (batch_size * seq_len, N_CONV_B))

    # 5.5. Dense layer
    l_2 = lasagne.layers.DenseLayer(l_reshape_b, num_units=N_L2,
